...
A visual way to see the same flow:
...
sequenceDiagram
    ...
    You->>DNS: Where does "example.com" live?
    DNS-->>You: Here's its IP address
    You->>Server: HTTP Request (GET /index.html)
    Server-->>You: HTTP Response (HTML, status code, headers)
    You->>Server: More requests for CSS, images, fonts
    Server-->>You: More responses
    Note over You: Page is now fully rendered
```
...
flowchart LR
    A["HTML — the skeleton
    (structure & meaning)"] -->|styled by| B["CSS — the skin
    (appearance & layout)"]
    B --> C["Same content,
    now looking intentional"]
```
